 ...
 *
 * `CARGO_HAULER_CARGO_BIN` is the explicit override; the legacy
 * `CARGO_CONDUCTOR_CARGO_BIN` remains a backward-compatible fallback so
 * existing operator setups keep their real cargo path. The per-job env is
...
export const realCargoBin = (
  ...
  const override =
    env.CARGO_HAULER_CARGO_BIN ??
    env.CARGO_CONDUCTOR_CARGO_BIN ??
    process.env.CARGO_HAULER_CARGO_BIN ??
    process.env.CARGO_CONDUCTOR_CARGO_BIN;
