...
const notice = (model: ReturnType<typeof daemonBadgeModel>): string => {
  switch (model.state) {
    case 'running':
      return `cargo-hauler ${model.headline}; ${model.detail ?? ''}.${model.skew === null ? '' : ` ${model.skew}.`} Before running cargo, check \`hauler status --session <id>\` (or the hauler_status tool with its session field) and attach to in-flight work instead of starting a duplicate; never kill cargo by PID — \`hauler kill cc-N\` stops a stuck ticket through the broker.`;
    case 'stopped':
