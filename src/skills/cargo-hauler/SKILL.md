...
- Long builds: `hauler exec --bg -- cargo …` or `hauler_request`.
  Wait once with `hauler await cc-N --max-wait-ms 7200000` (maximum two
  hours), or call `hauler_await` with the same `maxWaitMs`; do not hand-roll
  a tight `hauler_result` polling loop. `hauler result cc-N` /
